...
d'information. **Fonctionne entièrement hors-ligne** : les librairies (vis.js, three.js) et
les icônes SVG sont embarquées dans le projet, aucun accès internet n'est requis.
...
- **Clic droit** sur la zone de dessin : ajouter un élément
  ...
  - **Texte** : annotation **multiligne** affichée en pur texte (sans icône) — une fenêtre
    de saisie s'ouvre à la création (`Entrée` = nouvelle ligne, `Ctrl+Entrée` = valider,
    `Échap` = annuler) ; sans texte, le libellé par défaut « Texte N » est utilisé. Le
    renommage de l'élément (clic droit) utilise la même fenêtre multiligne.
...
- **Copier / coller un élément** : `Ctrl+C` copie la sélection, `Ctrl+V` colle au centre
  de la vue ; ou clic droit sur un élément « Copier cet élément », puis clic droit sur le
  fond « Coller ici » (colle à l'emplacement du clic). La collée recrée de nouveaux
  éléments : type et libellé personnalisé conservés, libellés par défaut renumérotés.
...
- **Vue 3D** (bouton « 3D ») : visualisation en perspective de la topologie (icônes,
  libellés, liens colorés et pointillés conservés). Glisser pour faire pivoter, molette
  pour zoomer, `Échap` (ou le bouton « 3D ») pour revenir en 2D. Vue de consultation :
  les modifications se font en 2D (la scène 3D est reconstruite à chaque activation).
...
- `Ctrl+C` copier la sélection · `Ctrl+V` coller (au centre de la vue)
...
- `Échap` annuler la création de lien / le geste en cours, quitter la vue 3D, fermer le
  menu, vider la recherche
...
`cloud`, `internet`, `hdd`, `usb`, `printer`, `bay`, `text`
...
js/three/three.min.js       librairie three.js (r147, UMD) embarquée (vue 3D)
specs.md              spécifications d'origine
```
